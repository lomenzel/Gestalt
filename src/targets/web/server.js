...
const WATCH_DIRS = (process.env.GESTALT_WATCH_DIRS || "")
  .split(":")
  .filter(Boolean);
const HTML_TEMPLATE_PATH =
  process.env.GESTALT_HTML_TEMPLATE || path.join(__dirname, "index.html");
...
if (WATCH_DIRS.length > 0) {
  ...
  function onFileChange(filename) {
    ...
    timer = setTimeout(() => {
      console.log(
        `[gestalt-dev] Change detected${filename ? ": " + filename : ""}, rebuilding...`,
      );
      cachedAppJS = null;
