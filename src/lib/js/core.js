class GestaltCore {
  ...
  constructor(effectCallback) {
    if (!effectCallback)
      throw new Error(
        "Effect callback is required to initialize the application",
      );
  ...
  get state() {
    console.warn(
      "[DEBUG] Accessing state directly is meant for debugging purposes. Consider using the view instead.",
    );
    return this.#state;
  }
